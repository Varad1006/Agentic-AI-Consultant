...
# 2. Add an invisible "bridge" node to safely split the traffic
def fan_out_bridge(state: ConsultantState):
    return state # Does nothing, just passes the data through
    
workflow.add_node("bridge", fan_out_bridge)

# 3. Build linear start
workflow.set_entry_point("planner")
workflow.add_edge("planner", "analyst")

# 4. The Circuit Breaker (Conditional Edge)
def route_after_analyst(state: ConsultantState):
...
workflow.add_conditional_edges(
    ...
    route_after_analyst,
    {
        "continue": "bridge",  # Safely routes to a single node
        "abort": END           # Aborts if there are API rate limit errors
    }
)

# 5. The Safe Fan-Out (Parallel Execution)
workflow.add_edge("bridge", "architect")
workflow.add_edge("bridge", "roi_calculator")

# 6. The Fan-In and Finish
